...
const val singleEndStrType1 =
    "一、为我的预测结果写一篇分析，分析要包含以下四点：" +
            ...
            "3.指数变化：指出指数倾斜方向，不强行过度解读。" +
            "4.观点推荐。" +
            "每个大点用（一、二、三、四、）等数字标识，每一个大内容点内，小内容点用（1.2.3.）等标识，任意内容点之间都要换行。" +
            "【硬性约束】所有子点只写核心干货，禁止额外修饰铺垫，不扩充细节，严格控制目标字数，写完自行压缩删减，不要冗余废话。" +
            "固定基本面+战术分析+指数变化+观点推荐四段结构，控制350-450字；无空行，数字英文无空格；文案简洁硬核。\n" +
            ///////////////
            "二、为这篇文章生成一个能够体现连红，对本场预测结果准确有信心的标题，" +
            "标题需要包含本场赛事名或者对阵双方队伍名，并且不带确定性结果，控制在12-25字，例如[连红延续本场研判信心足！西甲vs皇马]。\n" +
            ////////////////
...
const val singleEndStrType2 =
    "一、为我的预测结果写一篇分析，分析要包含以下四点：" +
            ...
            "固定基本面+战术分析+指数变化+观点推荐四段结构，控制350-450字；无空行，数字英文无空格；文案简洁硬核。\n" +
            ///////////////
            "二、为这篇文章生成一个能够体现连红、对本场预测结果有信心的标题，" +
            "标题需要包含本场赛事名或者对阵双方队伍名，并且不带确定性结果，控制在12-25字，例如[连红延续本场研判信心足！西甲vs皇马]。\n" +
            ////////////////
            "三、再给这段文章写一份50-60字的前瞻，要体现连红概率大，对本场赛事连红有信心，并且期待大家解锁购买这篇文章。\n" +
            //////////////
            "四、 全文不能有空白行，任意数字、英文左右不能有空白格，不要有任何无关提醒！\n" +
            //////////////
            "五、注意对分析、标题、前瞻的字数要求，不能有字数不对。\n" +
...
const val singleEndStrType3 =
    "一、为我的预测结果写一篇分析，分析要包含以下四点：" +
            "1.基本面：采集两队积分位次、近期赛场得失数据、主客场战力差异，剔除冗余描述。" +
            "2.战术分析：解析两队惯用攻防体系，评估球权控制偏好、攻防转换速率、阵容薄弱环节。" +
            "3.指数变化：依托上述给出的给出的盘口与赔率，客观评估机构赔付倾向，只做事实层面解读，拒绝过度主观推演。" +
            "4.观点推荐：综合基本面差距、战术克制关系、指数数据，最终结论匹配预测方向。" +
            "每个大点用（一、二、三、四、）等数字标识，每一个大内容点内，小内容点用（1.2.3.）等标识，任意内容点之间都要换行。" +
            "【硬性约束】所有子点只写核心干货，禁止额外修饰铺垫，不扩充细节，严格控制目标字数，写完自行压缩删减，不要冗余废话。" +
            "固定基本面+战术分析+指数变化+观点推荐四段结构，控制350-450字；无空行，数字英文无空格；文案简洁硬核。\n" +
            ///////////////
...
const val singleEndStrType4 =
    "一、为我的预测结果写一篇分析，分析要包含以下四点：" +
            "1.基本面：聚焦两队实时竞技状态，整合联赛积分排位走势、五轮攻防表现、主客场适配能力、核心阵容完整度，纯干货数据输出。" +
            "2.战术分析：深度拆解球队赛场打法，分析队伍传控节奏特点、攻防转换实战效率、临场攻防致命缺陷，贴合真实赛场表现。" +
            "3.指数变化：结合上述给出的盘口与赔率数据，客观分析数据倾斜逻辑，理性判断机构导向，不牵强解读走势。" +
            "4.观点推荐：结合两队硬实力差距、战术对位优劣、数据基本面，综合汇总得出本场核心研判，紧扣预设预测结果。" +
            "每个大点用（一、二、三、四、）等数字标识，每一个大内容点内，小内容点用（1.2.3.）等标识，任意内容点之间都要换行。" +
            "【硬性约束】所有子点只写核心干货，禁止额外修饰铺垫，不扩充细节，严格控制目标字数，写完自行压缩删减，不要冗余废话。" +
            "固定基本面+战术分析+指数变化+观点推荐四段结构，控制350-450字；无空行，数字英文无空格；文案简洁硬核。\n" +
            ///////////////
...
const val multiEndStrType2 =
    "为我的预测结果撰写一篇250字左右的赛事解析，" +
            "解析需要涵盖基本面研判、战术对位拆解、盘口数据解读、结论推导四个板块，" +
            "【硬性约束】所有板块只保留关键信息，杜绝多余描写，不增加无关细节，" +
            "严格把控总字数，完成后精简压缩，去除无效语句。" +
            "全文禁止出现空行，所有数字、英文前后不加空格，不附带额外提示！\n"

const val multiEndStrType3 =
    "围绕本场预测撰写一篇250字左右的赛事分析，" +
            "分析需要包含基本面数据、攻防战术对比、盘赔走势研判、赛果结论四个模块，" +
            "【硬性约束】全部内容只保留核心信息，禁止多余渲染，不额外拓展细节，" +
            "严控总字数，完成后精简压缩，剔除所有无效语句。" +
            "全文不允许空白行，所有数字、英文前后不加空格，不附带任何额外说明！\n"

const val multiEndStrType4 =
    "依据本场预测撰写一篇250字左右的赛事研判，" +
            "研判需要设置基本面概况、战术优劣对比、指数逻辑解读、最终观点四个板块，" +
            "【硬性约束】全部板块只留存核心信息，拒绝冗余修饰，不延展额外细节，" +
            "严格管控全文总字数，写完精简删减，清除废话。" +
            "全文不得有空行，所有数字、英文两侧不加空格，不附带任何额外备注！\n"
