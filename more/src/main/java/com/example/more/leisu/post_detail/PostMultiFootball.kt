...
import com.example.more.leisu.transToMultiFootballHandicapAiQuestion
import com.example.more.leisu.transToMultiFootballTotalAiQuestion
    ...
    fun loadAiQuestion() {
        ...
        PreMultiFootball.instance().selectedItemArray.let {
            it.forEachIndexed { index, league ->
                when (league.type) {
                    MultiFootballChoiceType.TypeHandicap -> {
                        totalQuestion += (index + 1).numberTransToChinese() + "、" +
                                (league as PreMultiFootballHandicapData).transToMultiFootballHandicapAiQuestion() + "\n"
                    }

                    MultiFootballChoiceType.TypeTotal -> {
                        totalQuestion += (index + 1).numberTransToChinese() + "、" +
                                (league as PreMultiFootballTotalData).transToMultiFootballTotalAiQuestion() + "\n"
                    }
            ...
            totalQuestion += (it.size + 1).numberTransToChinese() + "、" + "为这篇${it.size}串1文章生成一个能够体现连红与信心，并且不带确定性结果的标题，控制在15字以内。\n" +
                    ...
                    (it.size + 3).numberTransToChinese() + "、" + "给出的回答不能与之前已有的回答重复太多或相似太多！如果之前我已经发送过极度相似的问题，你就从不同角度重写一篇分析。"
