...
class TouchWindowService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        initWindow()
        floatingView.findViewById<View>(R.id.tv_test_touch).setOnClickListener {
            LiveEventBus.get<Any?>(EventBusInfo.FLOAT_WINDOW_TEST_TOUCH)
                .post("测试点击了")
        }
        floatingView.findViewById<View>(R.id.tv_sure).setOnClickListener {
            stopSelf()
        }
    }
...
//            layoutParams.format = PixelFormat.TRANSPARENT
    ...
    private fun moveWindow(){
        ...
        // 实现拖拽逻辑
        val dragArea =
            floatingView.findViewById<View?>(R.id.layout_container_touch_window) // 整个布局作为拖拽区域
    ...
    interface OnTestButtonClickedListener{
        fun onClick()
    }
}